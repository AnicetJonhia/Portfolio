...
export function ExperienceItem({
  ...
  return (
    <div className={`relative flex flex-col md:flex-row items-center w-full mb-12 ${isLeft ? "md:flex-row-reverse" : ""}`}>
      ...
      <div className="absolute left-0 md:left-1/2 w-4 h-4 rounded-full bg-primary shadow-[0_0_10px_rgba(0,0,0,0.1)] z-10 transform -translate-x-1/2 md:translate-x-[-8px] top-1 md:top-6"></div>

     
      <div className={`w-full md:w-1/2 pl-8 md:pl-0 ${isLeft ? "md:pr-12" : "md:pl-12"}`}>
        <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-primary/20 bg-card">
          <CardHeader className="p-5 pb-3">
           
            <CardTitle className="text-lg md:text-xl font-bold text-primary transition-colors">
              {title}
            </CardTitle>
